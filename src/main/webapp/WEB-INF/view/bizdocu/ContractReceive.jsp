...
<script src="js/bizdocu/ContractReceiveAction.js"></script>
<style>
.secukit {
    z-index: 1000;
}
    ...
    <script src="js/secukit-one/util/signatureutil.js" charset="utf-8"></script>
	<!-- /SecuKit One Assets -->
<!-- 자바스크립트 추가 영역  -->

<form id="frm1" name="frm1" method="post" action="bizdocu.ContractReceive.do">
	...
	<select id="blockCount" name="blockCount" style="width:50px; ">		
   	...
   	조회건수 :  <span style="font-weight:bold; color:#ff9a00;">${contSearchInfoVo.totalCount}</span>
						...
						<c:if test="${sessionVo.accountVo.accnt_cd != 'UR'}" >
							<span class="buttonFormOrange"  title="수신한 계약서의 내용을 검토완료 합니다." ><a href="#none" id="multiRcvState">검토완료</a></span>&nbsp;
							<span class="buttonFormOrange"  title="수신한 계약서에 전자서명을 합니다." ><a href="#none" id="multiSgnState">전자서명</a></span>&nbsp;
						</c:if>
						...
						<colgroup>
							...
							<tr>
								...
								<th>작성일<br />계약일</th>
								<th>계약금액</th>								
								<th class="last">계약서상태<br />(사용자상태)</th>
							</tr>
						...
						<c:forEach items="${contSearchInfoVoList }" var="contSearchInfoVoList" varStatus="cnt">
							<tr>
<input type="hidden" name="contSearchInfoVoList[${cnt.index}].gubun" value="${contSearchInfoVoList.gubun}" />
<input type="hidden" name="contSearchInfoVoList[${cnt.index}].company_cnt" value="${contSearchInfoVoList.company_cnt}" />
<input type="hidden" name="contSearchInfoVoList[${cnt.index}].b_stat" id="b_stat${cnt.index}" value="${contSearchInfoVoList.b_stat}" />
								<td>
...
<a href="#none" id="viewCont" onClick="javascript:ContractReceiveAction.viewCont('${contSearchInfoVoList.seq }','${contSearchInfoVoList.gubun }');" >${contSearchInfoVoList.title }
								</td>																
...
<c:otherwise>${contSearchInfoVoList.contract_dt }</c:otherwise>
								...
								<td>${contSearchInfoVoList.nowstat }<br />(${contSearchInfoVoList.b_stat })</td>
							</tr>
						</c:forEach>
						</table>
...
<form id="frm2" name="frm2" method="post">
	...
	<input type="hidden" name="contSearchInfoVo.accnt_no" value="${contSearchInfoVo.accnt_no }" />
	<input type="hidden" name="previousLink" id="previousLink" value="BUY" />
